...
class Generator:
    ...
    # def post_processor(init_image, generated_image):
    ...
    def __call__(
        ...
        # init_img = resize_image_with_height(init_img, resolution[resolution_mode])
        init_image['background'] = resize_image_with_height(init_image['background'].convert('RGB'), resolution[resolution_mode])
        ...
        gray_target_image = init_image['background']
        ...
        mask = mask_image.resize(init_img.size)
        ...
        canny_control_image_init = self.canny_image(init_image['background'])
        depth_control_image = self.depth_image(init_image['background'])

        canny_control_image_init = resize_image_with_height(canny_control_image_init, resolution[resolution_mode])
        depth_control_image = resize_image_with_height(depth_control_image, resolution[resolution_mode])
        mask = resize_image_with_height(mask, resolution[resolution_mode])
...
def get_demo(generate_handler, segment_handler):
    with gr.Blocks() as demo:
        ...
        gr.HTML(
            "<h1 style='text-align: center;'>Regional Style Transfer Demo</h1>",
        )
