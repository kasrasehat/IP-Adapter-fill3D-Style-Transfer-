...
class Generator:
    def __init__(self):
        ...
        controlnet = [controlnet_inpaint, controlnet_canny, controlnet_depth, controlnet_seg, controlnet_normal]
    ...
    def normal_image(self, image: Image.Image):
        ...
        np_image = (np.array(control_image) / 256).astype('uint8')
        control_image = self.new_method(image, np_image)
        return control_image
    ...
    def __call__(
        ...
        init_img = init_image
        mask = mask_image.resize(init_image.size, Image.Resampling.LANCZOS)
        ...
        control_guidance_end = [inpaint_endpoint,
                                canny_endpoint,
                                depth_endpoint,
                                seg_endpoint,
                                normal_endpoint,]
        inpaint_control_image = self.make_inpaint_condition(init_image, mask_image)
        canny_control_image_init = self.canny_image(init_image)
        depth_control_image = self.depth_image(init_image)
        normal_control_image = self.normal_image(init_image)
        seg_control_image = self.seg_image(init_image)
    ...
    with gr.Blocks() as demo:
        ...
        with gr.Row():
            ...
            with gr.Column():
                ...
                inpaint_endpoint = gr.Slider(
                    minimum=0.1,
                    maximum=1,
                    value=1,
                    label="inpaint endpoint",
                    interactive=True,
                    step=0.05,
                )
