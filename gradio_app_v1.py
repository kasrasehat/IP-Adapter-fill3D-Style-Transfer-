...
class SegmentHandler:
    ...
    def __get_segmentation_data(
        ...
        segmentations = self.new_method(image, service_type)
        mask_image = Image.fromarray(segmentations[0]['mask']).convert('L').convert('RGB')
        return mask_image, segmentations[0]['label']
    ...
    def __call__(
        ...
        image = image_edit_data["background"].convert('RGB')
        
        mask_image, label = self.__get_segmentation_data(
            image, service_type
        )
