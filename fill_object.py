...
import torch
import gradio as gr
from diffusers import StableDiffusionControlNetInpaintPipeline, ControlNetModel, UniPCMultistepScheduler, DPMSolverMultistepScheduler, EulerAncestralDiscreteScheduler
from transformers import pipeline
...
from ada_pallete import ada_palette
from segment import Location_detector
...
class Generator:
    def __init__(self):
        ...
        # "/home/kasra/realisticVisionV60B1_v60B1InpaintingVAE.safetensors"
        self.pipe = StableDiffusionControlNetInpaintPipeline.from_single_file(
        "/home/kasra/PycharmProjects/IP_Adapter/inpaint_sdxl_model/realisticVisionV60B1_v60B1InpaintingVAE.safetensors",  controlnet=controlnet, torch_dtype=torch.float16,
        # vae=vae
        ).to(device)
    ...
    def __call__(
        ...
        # init_image = fff
        # mask = self.pipe.mask_processor.blur(mask, blur_factor=8)
        ...
        elif sampler_type == "EulerAncestralDiscreteScheduler":
        ...
        canny_control_image_init = self.canny_image(fff)
        depth_control_image = self.depth_image(fff)
        normal_control_image = self.normal_image(fff)
        seg_control_image = self.seg_image(fff)
        mlsd_control_image = self.mlsd(fff).resize(main_image.size)
        location = Location_detector(main_image, np.array(mask_image.convert('L')))

        if location == 'wall' or location == 'ceiling' or generation_mode == 'force':    
            images = self.pipe(prompt=positive_prompt, 
        ...
        elif location == 'floor' and generation_mode != 'force':
            images = self.pipe(prompt=positive_prompt, 
                                    negative_prompt=negative_prompt,
                                    image=main_image,
                                    mask_image=mask,
        ...
        image = self.pipe.image_processor.apply_overlay(mask, main_image, images[0][0])
        image = image.resize(main_image.size)
        return image


def replace_btn_handler(output_image):
    layer_1 = np.zeros((output_image.height, output_image.width, 4), dtype=np.uint8)
    return {
...
def get_demo(generate_handler):
    with gr.Blocks() as demo:
        gr.HTML("<h1 style='text-align: center;'>3D fill</h1>",)
        with gr.Row():
            ...
            with gr.Column():
                ...
                negative_prompt = gr.Textbox(
                    "Low quality, bad quality, blur, blurry",
                    lines=3,
...
generation_mode
                ...
                sampler_type = gr.Dropdown(
                    ["UniPCMultistepScheduler", "DPMSolverMultistepScheduler", "DEISMultistepScheduler", "EulerAncestralDiscreteScheduler"],
                    label="Sampler Type",
        ...
        replace_btn.click(replace_btn_handler, [output_image], init_image)
